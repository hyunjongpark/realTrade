...
public class SocketCommunicator implements Runnable {
	...
	class TodayRealTimeSymbolType {
		...
		double continueUpProfit = 0;
		boolean isContinucUpFailed = true;
		double buyProfit = 0;
	...
	public void realTimeTradeManagerByVolumn() {
		...
		*/
		...
		while (isContinue) {
			...
			/*
			 * if (sellOrderList.size() > 0) { // 전날 보다 더 떨어지면 시장가로 매도 주문 try {
			 * // 매수한 종목이 매도되었는지 체크 List<SellOrder> removeSellOrderList = new
			 * ArrayList<SellOrder>(); for (SellOrder sell : sellOrderList) {
			 * boolean isFinishSell = true; for (TradeType type : bankList) { if
			 * (sell.symbol.equals(type.symbol)) { isFinishSell = false; } } if
			 * (isFinishSell) { removeSellOrderList.add(sell); } } if
			 * (removeSellOrderList.size() > 0) {
			 * sellOrderList.removeAll(removeSellOrderList); }
			 * removeSellOrderList.clear();
			 * 
			 * // 당일 매수한 종목 중 매도 안된 종목이 전말 보다 떨어지면 매도 주문 for (SellOrder sell :
			 * sellOrderList) { for (TradeType type : bankList) { if
			 * (type.symbol.equals(sell.symbol)) { for (int t = 0; t <
			 * todaySymbolList.size(); t++) { TodayRealTimeSymbolType
			 * tradeSymbol = todaySymbolList.get(t); if
			 * (!sell.symbol.equals(tradeSymbol.symbol)) { continue; } if
			 * (sell.symbol.equals(type.symbol) &&
			 * CommonUtil.getPecentPrice(tradeSymbol.yesterClosePrice, -2) >
			 * type.currentPrice) { removeSellOrderList.add(sell);
			 * getTradeCommunicator().requestModifyTrade(type.symbol,
			 * Integer.toString(type.buyCount),
			 * Double.toString(type.currentPrice), sell.number); try {
			 * Thread.sleep(5000); } catch (InterruptedException e) {
			 * e.printStackTrace(); }
			 * CommonUtil.writeToFile(TradeType.trade_path, " minus Sell " +
			 * mHour + ":" + mMinute + ":" + mSecond + " " + type.symbol +
			 * " price: " + type.currentPrice + " count: " +
			 * Integer.toString(type.buyCount) +
			 * " tradeSymbol.yesterClosePrice: " + tradeSymbol.yesterClosePrice
			 * + " num : " + sell.number + "\n"); } } } } } if
			 * (removeSellOrderList.size() > 0) {
			 * sellOrderList.removeAll(removeSellOrderList); } } catch
			 * (Exception e) { System.out.println(e); } }
			 * 
			 * 
			 * // 매수 한 종목은 바로 0.8% 올셔서 매도주문 if (todayBuyList.size() > 0) {
			 * List<String> removeBuyList = new ArrayList<String>(); for (String
			 * ss : todayBuyList) { for (TradeType type : bankList) { if
			 * (type.symbol.equals(ss)) { for (int t = 0; t <
			 * todaySymbolList.size(); t++) { TodayRealTimeSymbolType
			 * tradeSymbol = todaySymbolList.get(t); if
			 * (type.symbol.equals(tradeSymbol.symbol) &&
			 * tradeSymbol.isSellOrder == false) { tradeSymbol.isSellOrder =
			 * true; removeBuyList.add(tradeSymbol.symbol);
			 * 
			 * String n = orderSellToClient(type.symbol, type.buyCount,
			 * CommonUtil.getPecentPrice(type.buyPrice,
			 * VVIPManager.sellPercentByBuyPrice));
			 * CommonUtil.writeToFile(TradeType.trade_path, "Sell order " +
			 * mHour + ":" + mMinute + " " + type.symbol + " price: " +
			 * type.currentPrice + " count: " + Integer.toString(type.buyCount)
			 * + " num : " + n + "\n"); sellOrderList.add(new
			 * SellOrder(type.symbol, n, mHour, mMinute)); try {
			 * Thread.sleep(5000); } catch (InterruptedException e) {
			 * e.printStackTrace(); } } } } } }
			...
			for (int t = 0; t < todaySymbolList.size(); t++) {
				...
				// update pattern
				// QuoteList quoteList11 =
				// DatabaseManager.selectLimitQuoteListBySymbol(tradeSymbol.symbol,
				// 0, 50, true);
				// quoteList11.addQuote(currentQuote);
				// TechnicalAnalysis ta = new
				// TechnicalAnalysis(quoteList11.getList(),
				// quoteList11.getSize() - 1);
				// ta.setTaPattern();
				...
				checkArray.add(((currentPrice * diff) / 10000000) > 1);
				...
				if (isBuyStatus) {
					...
					tradeSymbol.isContinucUpFailed = false;
					tradeSymbol.buyProfit = currentPrice;
				...
				if (tradeSymbol.isBuyOrder && tradeSymbol.continueUpProfit <= currentProfit && tradeSymbol.isContinucUpFailed == false) {
					tradeSymbol.continueUpProfit = currentProfit;
					CommonUtil.writeToFile(TradeType.trade_path, "up:" + tradeSymbol.symbol + " maxProfit: " + (currentProfit - tradeSymbol.buyProfit));
					CommonUtil.writeToFile(TradeType.trade_path + "_" + tradeSymbol.symbol, "up:" + tradeSymbol.symbol + " maxProfit: " + (currentProfit - tradeSymbol.buyProfit));
				} else if (tradeSymbol.isBuyOrder && tradeSymbol.isContinucUpFailed == false) {
					tradeSymbol.isContinucUpFailed = true;
					CommonUtil.writeToFile(TradeType.trade_path, "down:" + tradeSymbol.symbol + " maxProfit: " + (currentProfit - tradeSymbol.buyProfit));
					CommonUtil.writeToFile(TradeType.trade_path + "_" + tradeSymbol.symbol, "down:" + tradeSymbol.symbol + " maxProfit: " + (currentProfit - tradeSymbol.buyProfit));

					for (String ss : todayBuyList) {
						if (ss.equals(tradeSymbol.symbol)) {
							for (TradeType type : bankList) {
								if (type.symbol.equals(tradeSymbol.symbol)) {
...
//									String n = orderSellToClient(type.symbol, type.buyCount, CommonUtil.getPecentPrice(type.buyPrice, VVIPManager.sellPercentByBuyPrice));
									String n = orderSellToClient(type.symbol, type.buyCount, 0);
					...
					todayBuyList.remove(tradeSymbol.symbol);
