...
function scrollMouseUp(evt) {
    ...
    if(scrollPos == 3)
        ...
        index = startIndex + Math.round((size - (dayCount+startIndex)) * (scrollPos-2) / (maxScrollPos-2));
...
function applyScroll() {
    if ( index == 0 )
        ...
        scrollPos = Math.round( ((index-startIndex) * maxScrollPos-2) / (size-(dayCount+startIndex))) + 2;
